...
class BalanceSheetForecastModel(tf.keras.Model):
    ...
    def call(
        ...
        # 1. Predict or override behavioral drivers
        if drivers_override is None:
            drivers = self.driver_head(features, training=training)
        else:
            # Basic shape validation (expect [B,T,1] per field matching features time dim)
            T_feat = tf.shape(features)[1]
            def _assert(name: str, x: tf.Tensor):
                with tf.control_dependencies([
                    tf.debugging.assert_equal(
                        tf.shape(x)[1], T_feat,
                        message=f"drivers_override.{name} T mismatch vs features"
                    )
                ]):
                    return tf.identity(x)
            # Force identity ops so assertions run in graph mode
            drivers = Drivers(
                price=_assert("price", drivers_override.price),
                volume=_assert("volume", drivers_override.volume),
                dso_days=_assert("dso_days", drivers_override.dso_days),
                dpo_days=_assert("dpo_days", drivers_override.dpo_days),
                dio_days=_assert("dio_days", drivers_override.dio_days),
                capex=_assert("capex", drivers_override.capex),
                stlt_split=_assert("stlt_split", drivers_override.stlt_split),
            )
