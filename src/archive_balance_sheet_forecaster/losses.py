...
def mae_masked(x: tf.Tensor, mask: tf.Tensor) -> tf.Tensor:
        """
        Mean Absolute Error with a 0/1 mask. Ignores NaNs in targets by default.

        Inputs:
            x: [B, T, 1] residuals (pred - true)
            mask: [B, T, 1] with 1.0 for valid entries, 0.0 to ignore
        """
        x_abs = tf.abs(x) * mask
        denom = tf.reduce_sum(mask) + 1e-8
        return tf.reduce_sum(x_abs) / denom
...
def statement_fit_loss(
    ...
    for key in candidate_keys:
        if key in true:
            ...
            tgt = true[key]
            # Build mask: ignore NaNs and infs in target
            mask = tf.cast(tf.math.is_finite(tgt), tf.float32)
            loss += w * mae_masked(pred_line - tgt, mask)
...
def identity_guardrail(pred: Statements) -> tf.Tensor:
    ...
    This would be applied in training with a TINY coefficient
