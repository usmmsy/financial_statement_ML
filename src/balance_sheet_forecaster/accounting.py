...
def clamp_positive(x: tf.Tensor, epsilon: float = 1e-6) -> tf.Tensor:
    """
...
class StructuralLayer(tf.keras.layers.Layer):
    ...
    def call(self,
        ...
        st_rate = policies.st_borrow_rate if getattr(policies, "st_rate", None) is not None else nominal_rate
        ...
        cost_share_policy = getattr(policies, "cost_share", None)
        ...
        # Inventory / COGS modeling via DIO
        #   implied_inventory ~ sales * dio_days/365 * cost_share
        #   COGS ~ implied_inventory * 365 / dio_days
        # In production, should replace with a FIFO inventory engine.
        implied_inventory = days_to_balance(sales, dio) * cost_share # [B, T, 1]
        cogs = clamp_positive(
            implied_inventory * 365.0 / clamp_positive(dio)
        )   # [B, T, 1]
        ...
        if cash_cov is not None:
            min_cash = tf.nn.relu(cash_cov) * (cogs + opex + tax) * period_frac
        else:   # [B, T, 1]
        ...
        fcf = cfo - capex_out - dividends    # free cash flow, [B, T, 1]
